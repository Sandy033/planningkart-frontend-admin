...
// ─── Single Event Card ────────────────────────────────────────────────────────
const EventListCard = ({ event, onToggleStatus, categoryName }) => {
    ...
    return (
        <div className="el-card">
            {/* Image / Carousel */}
            {images.length > 0
                ? <ImageCarousel images={images} title={event.name || event.title} />
            ...
            <div className="el-body">
                <h4 className="el-title">{event.name || event.title || 'Untitled Event'}</h4>

                {event.description && (
                    <p className="el-short-desc">{event.description}</p>
                )}
...
// ─── Event List ───────────────────────────────────────────────────────────────
const EventList = ({ events, onToggleStatus }) => {
    const { items: categories } = useSelector((state) => state.categories);

    const getCategoryName = (event) => {
        const catId = event.categoryId || event.eventCategory?.id || event.category?.id;
